...
async function migrateUsers(sourceDb, targetDb) {
  ...
  for (const user of betterAuthUsers) {
    try {
      ...
      console.log(`${DRY_RUN ? '✓' : '✅'} ${email} [${user.name || 'No name'}]${hasPassword ? ' 🔑' : ''}`);
      migrated++;
...
async function main() {
  ...
  try {
    ...
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
    console.log(`   👥 Users:        ${userStats.migrated} ${DRY_RUN ? '(would be migrated)' : 'migrated'}`);
    console.log(`   📦 Collections:  ${dataStats.collectionsMigrated} ${DRY_RUN ? '(would be migrated)' : 'migrated'}`);
    console.log(`   📄 Documents:    ${dataStats.totalDocs} ${DRY_RUN ? '(would be migrated)' : 'migrated'}`);
    console.log(`   📊 Grand Total:  ${userStats.migrated + dataStats.totalDocs} ${DRY_RUN ? '(would be migrated)' : 'migrated'}\n`);
