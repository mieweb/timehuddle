...
const TARGET_MONGO_URL =
  process.env.TARGET_MONGO_URL || process.env.MONGO_URL || 'mongodb://127.0.0.1:27017/timehuddle';
...
async function migrateUsers() {
  ...
  for (const user of betterAuthUsers) {
    try {
      ...
      if (DRY_RUN) {
        console.log(
          `✓ Would migrate: ${email} [${user.name || 'No name'}]${hasPassword ? ' 🔑' : ' ⚠️ no password'}`,
        );
      } else {
        await targetDb.collection('users').insertOne(meteorUser);
        console.log(
          `✅ Migrated: ${email} [${user.name || 'No name'}]${hasPassword ? ' 🔑' : ' ⚠️ no password'}`,
        );
      }
      migrated++;
    } catch (error) {
