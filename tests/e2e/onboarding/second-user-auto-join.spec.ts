...
test.describe('New User Auto-Join', () => {
  test('new user is auto-joined to default org and does not see installer modal', async ({
    page,
  }) => {
    ...
    await expect(page.getByText(new RegExp(`${uniqueName} User`, 'i'))).toBeVisible({
      timeout: 5000,
    });
  });
});
