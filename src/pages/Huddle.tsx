...
export default function Huddle() {
  ...
  function addPost(content: ComposerContent) {
    try {
    ...
    if (!user) return; // Guard: don't post if not logged in
    
    const newPost: Post = {
      id: Date.now().toString(),
      author: user.name,
      initials: getUserInitials(user.name),
      avatarColor: getUserColor(user.id),
      time: 'Just now',
      body: content.text,
      ticket: content.ticketId ? {
        id: content.ticketId,
        title: `#${content.ticketId} — Ticket`,
        status: 'Open',
        time: '0m',
        assignee: user.name.split(' ')[0] + ' ' + user.name.split(' ')[1]?.[0] + '.',
      } : undefined,
      likes: 0,
      views: 1,
      comments: [],
    ...
    if (content.attachments.length > 0) {
      console.log('[Huddle] Processing attachments, count:', content.attachments.length);
      const firstAttachment = content.attachments[0];
      console.log('[Huddle] First attachment:', firstAttachment);
      // For now, just show the first image attachment as the post image
      if (firstAttachment.type === 'image' || firstAttachment.mimeType?.startsWith('image/')) {
        console.log('[Huddle] Setting imageUrl:', firstAttachment.url);
        newPost.imageUrl = firstAttachment.url;
      }
    } else {
      console.log('[Huddle] No attachments in content');
    }
    
    console.log('[Huddle] Final newPost:', newPost);
