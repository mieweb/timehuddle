          ...
          if (tab.isFab) {
            return (
              <button
                ...
                className="relative -top-4 flex h-16 w-16 flex-col items-center justify-center rounded-full shadow-lg transition-transform active:scale-95"
                style={{
                  background:
                    'linear-gradient(135deg, var(--color-primary-400, #60a5fa), var(--color-primary-600, #2563eb))',
                  boxShadow: '0 4px 18px 0 color-mix(in srgb, var(--color-primary, #3b82f6) 45%, transparent)',
                }}
              >
                <FontAwesomeIcon icon={tab.icon} className="text-xl text-white" />
                <span className="mt-0.5 text-[9px] font-medium text-white/90">{tab.label}</span>
              </button>
            );
          }
