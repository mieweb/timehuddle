...
export async function verifyWsToken(token: string | undefined): Promise<AppUser | null> {
  ...
  if (meteorUser) {
    ...
    if (fastifyUser) {
      return { id: fastifyUser._id.toString(), name: fastifyUser.name, email: fastifyUser.email, image: fastifyUser.image };
    }
    return { id: meteorUser._id.toString(), name: meteorUser.profile?.name ?? email, email, image: null };
  }
