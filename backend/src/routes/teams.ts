...
export async function teamRoutes(app: FastifyInstance) {
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      if (result === "not-found") return (reply as any).status(404).send({ error: "Team not found" });
      if (result === "already-member") return (reply as any).status(409).send({ error: "Already a member" });
  ...
  app.delete(
    ...
    async (req, reply) => {
      ...
      if (result === "not-member") return (reply as any).status(400).send({ error: "User is not a member" });
