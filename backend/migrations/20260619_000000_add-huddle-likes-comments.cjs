...
module.exports = {
  async up(db) {
    ...
    try {
      ...
      const hasIndex = existingIndexes.some(
        (idx) => idx.key && idx.key.postId === 1 && idx.key.createdAt === 1
      );

      if (!hasIndex) {
        await db
          .collection("huddlecomments")
          .createIndex({ postId: 1, createdAt: 1 }, { name: "postId_createdAt" });
      }
    } catch (err) {
    ...
    await db
      ...
      .updateMany({ likes: { $exists: false } }, { $set: { likes: [], commentCount: 0 } });
  ...
  async down(db) {
    ...
    await db.collection("huddleposts").updateMany({}, { $unset: { likes: "", commentCount: "" } });
