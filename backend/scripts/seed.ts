...
const SEED_TICKETS: {
  ...
  // Developers
  { team: "Developers", title: "Set up CI/CD pipeline", status: "closed", priority: "high", createdBy: "alice@example.com", assignedTo: "bob@example.com" },
  { team: "Developers", title: "Refactor timer state models", status: "in-progress", priority: "high", createdBy: "alice@example.com", assignedTo: "carol@example.com" },
  { team: "Developers", title: "Add dark mode support", status: "open", priority: "medium", createdBy: "carol@example.com", assignedTo: "dan@example.com" },
  { team: "Developers", title: "Fix mobile layout on timers page", status: "open", priority: "medium", createdBy: "bob@example.com", assignedTo: "eve@example.com" },
  { team: "Developers", title: "Write unit tests for timer service", status: "in-progress", priority: "high", createdBy: "carol@example.com", assignedTo: "ian@example.com" },
  { team: "Developers", title: "Implement push notifications", status: "open", priority: "low", createdBy: "alice@example.com", assignedTo: null },
  { team: "Developers", title: "Upgrade to Vite 8", status: "closed", priority: "medium", createdBy: "dan@example.com", assignedTo: "dan@example.com" },
  // Product
  { team: "Product", title: "Define MVP feature set", status: "closed", priority: "critical", createdBy: "kira@example.com", assignedTo: "kira@example.com" },
  { team: "Product", title: "Write user stories for time tracking", status: "reviewed", priority: "high", createdBy: "kira@example.com", assignedTo: "alice@example.com" },
  { team: "Product", title: "Roadmap Q3 2026", status: "in-progress", priority: "medium", createdBy: "kira@example.com", assignedTo: "liam@example.com" },
  { team: "Product", title: "Accessibility audit", status: "open", priority: "medium", createdBy: "liam@example.com", assignedTo: null },
  // Design
  { team: "Design", title: "Design system token alignment", status: "in-progress", priority: "high", createdBy: "liam@example.com", assignedTo: "liam@example.com" },
  { team: "Design", title: "Timer page mobile mockups", status: "reviewed", priority: "medium", createdBy: "liam@example.com", assignedTo: "maya@example.com" },
  { team: "Design", title: "Onboarding flow illustrations", status: "open", priority: "low", createdBy: "maya@example.com", assignedTo: null },
  // Support
  { team: "Support", title: "Document clock-in / clock-out flow", status: "closed", priority: "medium", createdBy: "jules@example.com", assignedTo: "hannah@example.com" },
  { team: "Support", title: "Build FAQ knowledge base", status: "in-progress", priority: "medium", createdBy: "jules@example.com", assignedTo: "noah@example.com" },
  // Accounting
  { team: "Accounting", title: "Monthly payroll reconciliation", status: "in-progress", priority: "high", createdBy: "frank@example.com", assignedTo: "grace@example.com" },
  { team: "Accounting", title: "Set up invoice export", status: "open", priority: "medium", createdBy: "grace@example.com", assignedTo: "frank@example.com" },
  // Operations
  { team: "Operations", title: "Docker Compose local dev stack", status: "closed", priority: "high", createdBy: "noah@example.com", assignedTo: "ian@example.com" },
  { team: "Operations", title: "Environment rotation script", status: "open", priority: "low", createdBy: "ian@example.com", assignedTo: null },
];
...
// clockOutHour/Min = 0/0 means still clocked in (open event)
const CLOCK_SCHEDULE_BASE: [string, number, number, number, number, number][] = [
  ...
  ["eve@example.com", 0, 8, 30, 0, 0],
];

const CLOCK_SCHEDULE: [string, number, number, number, number, number][] = THREE_WEEK_OFFSETS.flatMap(
  (weekOffset) =>
...
const TIMER_SEEDS_BASE: TimerSeed[] = [
  ...
  ["alice@example.com", 4, "Refactor timer state models", "Kickoff — schema design with team", [[9, 0, 10, 30]]],
  ["alice@example.com", 4, "Refactor timer state models", "TimeEntry model draft", [[11, 0, 12, 30], [13, 30, 15, 0]]],
  ["alice@example.com", 3, "Refactor timer state models", "TimerSession model + migration script", [[9, 0, 11, 30]]],
  ["alice@example.com", 3, "Refactor timer state models", "Backend routes — POST /timer-sessions", [[13, 0, 16, 0]]],
  ["alice@example.com", 2, "Refactor timer state models", "PATCH route + auth guards", [[8, 30, 11, 0]]],
  ["alice@example.com", 2, "Refactor timer state models", "PR cleanup and rebase", [[14, 0, 15, 30]]],
  ["alice@example.com", 1, "Refactor timer state models", "Review carol's test feedback", [[9, 0, 10, 0]]],
  ...
  ["carol@example.com", 3, "Refactor timer state models", "Read alice's PR — understand new model shape", [[8, 0, 9, 0]]],
  ["carol@example.com", 3, "Refactor timer state models", "Unit tests for TimeEntry service", [[9, 30, 12, 0]]],
  ["carol@example.com", 2, "Refactor timer state models", "Integration tests — session lifecycle", [[8, 0, 11, 30]]],
  ["carol@example.com", 2, "Refactor timer state models", "Edge cases: concurrent sessions, clock drift", [[13, 0, 15, 0]]],
  ["carol@example.com", 1, "Refactor timer state models", "Fix broken assertion after alice's rebase", [[8, 0, 9, 30]]],
  // bob: one-day review pass
  ["bob@example.com", 1, "Refactor timer state models", "Code review pass", [[11, 0, 12, 30]]],
  ["bob@example.com", 1, "Refactor timer state models", "Left review comments — API surface concerns", [[14, 0, 15, 0]]],
  ...
  ["bob@example.com", 4, "Set up CI/CD pipeline", "Deploy job + staging environment hook", [[13, 0, 15, 30]]],
  ["alice@example.com", 3, "Set up CI/CD pipeline", "Test gate — block merge on red", [[9, 0, 10, 0]]],
  ...
  ["bob@example.com", 4, "Fix mobile layout on timers page", "Breakpoint analysis — document current issues", [[15, 0, 17, 0]]],
  ["bob@example.com", 3, "Fix mobile layout on timers page", "Responsive table — collapse note column", [[9, 0, 11, 0], [13, 30, 15, 30]]],
  ["bob@example.com", 2, "Fix mobile layout on timers page", "Week strip overflow fix", [[9, 0, 12, 30]]],
  ["eve@example.com", 2, "Fix mobile layout on timers page", "QA pass on iPhone 15 Pro", [[13, 0, 14, 30]]],
  ["eve@example.com", 1, "Fix mobile layout on timers page", "Testing on Android (Pixel 8)", [[8, 45, 12, 0]]],
  ["dan@example.com", 1, "Fix mobile layout on timers page", "Accessibility check — tab order", [[14, 30, 16, 0]]],
  ["bob@example.com", 0, "Fix mobile layout on timers page", "Final polish — PR ready", [[9, 10, 11, 0]]],

  // ── "Add dark mode support" ── carol + dan share the work
  ["dan@example.com", 4, "Add dark mode support", "Token audit", [[10, 0, 12, 0]]],
  ["carol@example.com", 4, "Add dark mode support", "Identify components missing dark variants", [[13, 0, 15, 30]]],
  ["dan@example.com", 3, "Add dark mode support", "Dark palette tokens — map to Tailwind vars", [[10, 0, 13, 0]]],
  ["carol@example.com", 3, "Add dark mode support", "AppHeader + Sidebar dark mode", [[13, 0, 14, 30]]],
  ...
  ["dan@example.com", 1, "Add dark mode support", "Review fixes after carol's audit", [[10, 0, 12, 0], [14, 0, 17, 30]]],

  // ── "Write unit tests for timer service" ── carol primary, alice assists, ian chips in
  ["carol@example.com", 4, "Write unit tests for timer service", "Timer session tests — start/stop/resume", [[8, 0, 11, 0], [13, 0, 15, 0]]],
  ["alice@example.com", 3, "Write unit tests for timer service", "Route tests — POST /time-entries", [[9, 0, 11, 0], [14, 0, 16, 30]]],
  ["carol@example.com", 3, "Write unit tests for timer service", "Service layer — createSession edge cases", [[8, 30, 10, 0]]],
  ["ian@example.com", 3, "Write unit tests for timer service", "", [[14, 0, 16, 0]]],  // no note
  ["carol@example.com", 1, "Write unit tests for timer service", "Integration tests — full lifecycle", [[8, 0, 10, 0], [13, 30, 16, 0]]],
  ["ian@example.com", 1, "Write unit tests for timer service", "CI runner config for backend tests", [[10, 0, 12, 0]]],

  // ── "Upgrade to Vite 8" ── dan leads, bob assists
  ["dan@example.com", 4, "Upgrade to Vite 8", "Research — changelog + breaking changes", [[10, 0, 12, 0]]],
  ["bob@example.com", 3, "Upgrade to Vite 8", "Dependency bump + build smoke test", [[9, 0, 10, 30], [14, 0, 17, 0]]],
  ...
  ["eve@example.com", 4, "Implement push notifications", "Push subscription API research", [[13, 0, 15, 0]]],
  ["eve@example.com", 3, "Implement push notifications", "FCM integration — server-side send", [[8, 30, 12, 0]]],
  ["eve@example.com", 3, "Implement push notifications", "Client-side permission prompt", [[13, 0, 15, 0]]],
  ["eve@example.com", 0, "Implement push notifications", "Subscription flow — persistence + re-subscribe", [[8, 30, 10, 30]]],

  // ── Product tickets — kira + liam
  ["kira@example.com", 4, "Define MVP feature set", "", [[9, 0, 10, 0]]],  // no note
  ["kira@example.com", 4, "Define MVP feature set", "Final sign-off with stakeholders", [[10, 30, 12, 0]]],
  ["kira@example.com", 3, "Roadmap Q3 2026", "Draft roadmap", [[9, 0, 12, 0], [13, 0, 15, 0]]],
  ["liam@example.com", 3, "Roadmap Q3 2026", "Design capacity estimates", [[9, 30, 11, 0]]],
  ["kira@example.com", 2, "Write user stories for time tracking", "Story mapping session", [[9, 0, 11, 30]]],
  ["kira@example.com", 2, "Write user stories for time tracking", "AC writeup for timer entries", [[13, 0, 14, 30]]],
  ["liam@example.com", 2, "Write user stories for time tracking", "Review kira's AC — design notes", [[14, 30, 16, 0]]],
  ["kira@example.com", 1, "Roadmap Q3 2026", "Stakeholder review — slides", [[9, 30, 12, 0]]],
  ["kira@example.com", 0, "Accessibility audit", "Screen reader pass on timer page", [[9, 0, 11, 0]]],

  // ── Design tickets — liam + carol spotcheck
  ["liam@example.com", 4, "Design system token alignment", "Audit existing tokens — spreadsheet", [[9, 0, 12, 0]]],
  ["liam@example.com", 4, "Design system token alignment", "Map brand tokens to Tailwind vars", [[13, 0, 15, 0]]],
  ["liam@example.com", 3, "Timer page mobile mockups", "Wireframes — day view + week strip", [[9, 0, 11, 0], [14, 0, 16, 0]]],
  ["carol@example.com", 3, "Timer page mobile mockups", "Dev review — feasibility notes", [[11, 30, 12, 30]]],
  ["liam@example.com", 2, "Design system token alignment", "Token PR — submit for review", [[9, 0, 17, 0]]],
  ["liam@example.com", 1, "Timer page mobile mockups", "Incorporate dev feedback", [[9, 0, 11, 30]]],
  ...
  ["frank@example.com", 4, "Monthly payroll reconciliation", "Import payroll data from ADP", [[9, 0, 11, 0]]],
  ["frank@example.com", 4, "Monthly payroll reconciliation", "Spot-check overtime hours", [[13, 0, 15, 0]]],
  ["grace@example.com", 4, "Monthly payroll reconciliation", "Verify totals against source", [[9, 0, 12, 0]]],
  ["grace@example.com", 4, "Monthly payroll reconciliation", "Escalate 3 discrepancies", [[13, 30, 14, 30]]],
  ["frank@example.com", 3, "Set up invoice export", "Schema design — CSV + PDF targets", [[9, 0, 11, 0]]],
  ["grace@example.com", 3, "Set up invoice export", "Export testing with sample data", [[9, 0, 11, 30]]],
  ["grace@example.com", 3, "Set up invoice export", "Edge case: zero-hour employees", [[13, 0, 14, 0]]],
  ["frank@example.com", 2, "Monthly payroll reconciliation", "Final reconciliation run", [[9, 0, 16, 0]]],
  ["frank@example.com", 2, "Monthly payroll reconciliation", "Submit to payroll provider", [[16, 30, 17, 0]]],

  // ── Operations — ian + noah, one shared ticket
  ["noah@example.com", 4, "Docker Compose local dev stack", "Initial compose file", [[9, 0, 12, 0]]],
  ["ian@example.com", 4, "Docker Compose local dev stack", "MongoDB + Fastify services", [[13, 0, 17, 0]]],
  ["noah@example.com", 3, "Docker Compose local dev stack", "Seed integration — run seed on first boot", [[9, 0, 11, 0]]],
  ["ian@example.com", 3, "Docker Compose local dev stack", "", [[11, 30, 14, 0]]],  // no note
  ["ian@example.com", 2, "Environment rotation script", "Draft bash script for env swap", [[10, 0, 13, 0]]],
];
