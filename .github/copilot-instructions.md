...
**The component rules live in [`.github/instructions/mieweb-ui.instructions.md`](instructions/mieweb-ui.instructions.md)**, generated by `npx @mieweb/ui@0.8.0 init-agent` and applied automatically to `**/*.{ts,tsx,js,jsx}`. The same rules are mirrored into the `AGENTS.md` managed block for tools that read that convention.

That file is the single source of truth for component selection — it ships with the library and covers the full catalog, `ButtonGroup` usage, composition slots, variants, design tokens, and theming. Do not restate its rules here; they go stale as the library moves.

**Rerun `npx @mieweb/ui@<version> init-agent` after every `@mieweb/ui` upgrade** to refresh both targets, and commit the result.

The version is pinned in that command on purpose: the `init-agent` CLI does not exist before 0.8.0, and this project is held at 0.7.3 because 0.8.0+ regresses the Kerebron `RichEditor` seeding that the clock composer depends on. `npx` fetches the CLI without installing it, so the ruleset stays current even while the dependency cannot move.
