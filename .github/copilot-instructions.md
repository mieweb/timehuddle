...
**The component rules live in [`.github/instructions/mieweb-ui.instructions.md`](instructions/mieweb-ui.instructions.md)**, generated by `npx mieweb-ui init-agent` and applied automatically to `**/*.{ts,tsx,js,jsx}`. The same rules are mirrored into the `AGENTS.md` managed block for tools that read that convention.
...
**Rerun `npx mieweb-ui init-agent` after every `@mieweb/ui` upgrade** to refresh both targets, and commit the result.
